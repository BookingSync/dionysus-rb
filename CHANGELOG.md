...
## [1.6.0]
- Rank duplicate messages in a consumer batch by `updated_at` first, then `serialized_at`, then offset. 1.2.0 ranked on `[serialized_at, offset]` to identify the last-serialized payload; the consumer then applies `persist_with_dionysus?`, which compares the payload's `updated_at` against what is stored. Those are different questions, and when the last-serialized message is the one carrying a stale `updated_at`, ranking elects a message the guard immediately rejects - and the `has_many` records embedded in it are persisted further down the same loop body, so they are dropped with it. Sibling messages carrying the same data with an acceptable timestamp are already gone by then.
- Why `updated_at` is the right primary key to rank on. It is the field the guard actually compares, and it is monotonic on the row, so the group maximum is accepted whenever any message in the group would be accepted. That property is what makes the wholesale drop impossible: election can no longer pick a loser while a winner sits in the same batch.
- Why ranking cannot be fooled in the other direction. A serializer reads the record's own columns first and queries its associations afterwards, so a torn payload's `updated_at` is stale relative to its own contents, never fresher. There is no payload whose timestamp overstates how fresh its data is.
- `serialized_at` is kept as the tie-break, which preserves 1.2.0 exactly. `updated_at` does not move when only an associated record changes, so two payloads with different computed values can share one - that tie is precisely what 1.2.0 was written for, and it is still broken by the stamp. The offset breaks the remaining ties.
- Unchanged fallbacks: a group in which any message lacks a usable `serialized_at` still ranks on the offset alone, and a group in which any payload lacks a usable `updated_at` still ranks on `[serialized_at, offset]`. Models without timestamps, such as LOS records, are therefore unaffected.
- Measured in production on 2026-08-31, with `publish_consistent_snapshots` and `republish_deduplicated_records` both live: 2.79% of multi-message groups on `v3_bookings` still had the elected message report a timestamp older than a sibling already published at a lower offset, and a reconcile against Core over the same window found 2 of 373 payment-active bookings holding no payment at all. Booking 22636772 is the regression spec: three `with:` dependencies - BookingsPayment, Payment and BookingRevenueDistribution - each republished the same booking payload, two carried `updated_at 13:12:56.975713` and would have been accepted, and the third, serialized 250ms later off a stale parent read, carried `13:12:56.598805` and lost the guard by 15ms. It won election and the money was discarded.
- This needs no producer change and no config. The payload shape is unchanged; only which message of a group survives deduplication is different.
## [1.5.0]
