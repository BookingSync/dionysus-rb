...
RSpec.describe Dionysus::Producer::Outbox::HealthCheck do
  let(:healthcheck_storage) do
    FileBasedHealthcheck.new(directory: directory, filename: key, time_threshold: 120)
  end
  let(:key) { "__dionysus_outbox_worker__running__hostname" }
  let(:directory) { "/tmp" }
  ...
  describe ".check" do
    ...
    context "when the heartbeat has not been registered" do
      let(:expected_result) { "[Dionysus Producer Outbox healthcheck failed]" }
