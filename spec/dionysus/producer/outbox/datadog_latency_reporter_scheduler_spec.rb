...
RSpec.describe Dionysus::Producer::Outbox::DatadogLatencyReporterScheduler do
  describe "#add_to_schedule" do
    ...
    context "when the job does not exist" do
      let(:created_job) do
        Sidekiq::Cron::Job.find(name: "dionysus_producer_outbox_datadog_latency_reporter_job")
      end
      let(:description) do
        "Collect latency metrics from dionysus outbox and send them to Datadog"
      end
      ...
      it "adds a new job to the schedule" do
        ...
        expect(created_job.name).to eq "dionysus_producer_outbox_datadog_latency_reporter_job"
        expect(created_job.cron).to eq "* * * * *"
        expect(created_job.klass).to eq "Dionysus::Producer::Outbox::DatadogLatencyReporterJob"
        expect(created_job.queue_name_with_prefix).to eq "dionysus_high_priority"
